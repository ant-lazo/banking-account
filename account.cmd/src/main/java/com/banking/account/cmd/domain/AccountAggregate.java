...
public class AccountAggregate extends AggregateRoot {
    ...
    public AccountAggregate(OpenAccountCommand command){
        raiseEvent(AccountOpenendEvent.builder()
                .id(command.getId())
    ...
    public void apply(AccountOpenendEvent event){
        this.id = event.getId();
        this.active = true;
    ...
    public void depositFunds(double amount){
        ...
        if(amount<=0){
            throw new IllegalStateException("El deposito del dinero no puede ser cero o menos que cero");
        }
